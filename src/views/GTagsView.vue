<template>
    <div>
        <div class="flex justify-between items-center">
            <h1 class="font-bold lg:text-2xl text-xl text-gray-500 mb-5"> {{ nameGtag }}</h1>
            <button
...
export default {
    ...
    setup() {
        const nameGtag = store.getters.getItemName('Grupos etiquetas')
        const items = ref([]);
        ...
        const updateItems = async (data) => {
            ...
            try {
                ...
                const id = data._id;
                const payload = { name: data.name, mobilesItems: data.mobilesItems.map(item => item._id) };
                await updateGTagsApi(token, payload, id);
                closeEditForm();
                closeLoading();
                await fetchItems();
                Swal.fire({ title: 'Éxito', text: 'El grupo de etiquetas fue actualizado correctamente.', icon: 'success', confirmButtonText: 'Aceptar' });
            } catch (error) {
                closeLoading();
                const errorMessage = error.response?.data?.message || error.response?.data || 'Error al actualizar el grupo de etiquetas.';
        ...
        const submitItems = async (data) => {
            try {
                ...
                await fetchItems();
                closeCreateForm();
                Swal.fire({ title: 'Éxito', text: 'El grupo de etiquetas fue creado correctamente.', icon: 'success', confirmButtonText: 'Aceptar' });
            } catch (error) {
                closeCreateForm();
                const errorMessage = error.response?.data?.message || error.response?.data || 'Error al crear el grupo de etiquetas.';
                Swal.fire({ title: 'Error', text: errorMessage, icon: 'error', confirmButtonText: 'Aceptar' });
            }
        };
        ...
        const deleteItems = async (item) => {
            ...
            try {
                const token = store.state.token;
                const id = item._id;
                await deleteGTagsApi(token, id);
                closeLoading();
                await fetchItems();
                Swal.fire({ title: 'Éxito', text: 'El grupo de etiquetas fue eliminado correctamente.', icon: 'success', confirmButtonText: 'Aceptar' });
            } catch (error) {
                closeLoading();
                const errorMessage = error.response?.data?.message || error.response?.data || 'Error al eliminar el grupo de etiquetas.';
                Swal.fire({ title: 'Error', text: errorMessage, icon: 'error', confirmButtonText: 'Aceptar' });
            }
        };
        ...
        const fetchItems = async () => {
            ...
            try {
                ...
                closeLoading();
                const errorMessage = error.response?.data?.message || error.response?.data || 'Error al listar los grupos de etiquetas.';
                Swal.fire({ title: 'Error', text: errorMessage, icon: 'error', confirmButtonText: 'Aceptar' });
            }
        };
