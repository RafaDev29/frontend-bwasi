<template>
    <div>
        <div class="flex justify-between items-center">
            <h1 class="font-bold lg:text-2xl text-xl text-gray-500 mb-5"> Clientes</h1>
            <button
        ...
        <createForm v-if="isCreateFormVisible" @close="closeCreateForm"  />
        <EditForm v-if="isEditFormVisible" :item="selectedItem" @close="closeEditForm" />
    </div>
...
import { listCustomersApi, deleteCustomersApi, createCustomersApi, updateCustomersApi } from '@/api/CompanyService'
import formTable from '@/components/company/DataTable.vue';
import createForm from '@/components/company/CreateForm.vue';
import EditForm from '@/components/company/EditForm.vue';
    ...
    setup() {
        ...
        const updateItems = async (data) => {
            ...
            try {
                ...
                await updateCustomersApi(token, payload, id);
                closeEditForm();
                closeLoading();
                await fetchItems();
        ...
        const submitItems = async (data) => {
            try {
                const token = store.state.token;
                await createCustomersApi(token, data);
                await fetchItems();
                closeCreateForm();
        ...
        const deleteItems = async (item) => {
            ...
            try {
                const token = store.state.token;
                const id = item._id;
                await deleteCustomersApi(token, id);
                closeLoading();
                await fetchItems();
