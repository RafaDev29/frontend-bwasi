<template>
    ...
    <div class="flex justify-between items-center">
        <h1 class="font-bold lg:text-2xl text-xl text-gray-500 mb-5"> {{ NameLayout }}</h1>
        <button
    ...
    setup() {
        ...
        const updateItems = async (data) => {
            ...
            try {
                ...
                await updateLayoutApi(token, payload, id);
                closeEditForm();
                closeLoading();
                await fetchItems();
        ...
        const fetchItems = async () => {
            ...
            try {
                ...
                closeLoading();
                const errorMessage = error.response?.data?.message || error.response?.data || 'Error al obtener los datos.';
                Swal.fire({ title: 'Error', text: errorMessage, icon: 'error', confirmButtonText: 'Aceptar' });
            }
        };
