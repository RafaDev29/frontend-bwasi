<template>
    <div v-if="item" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-[9999] ">
        <div class="bg-white rounded-lg shadow-2xl max-w-4xl w-full p-8 max-h-[95vh] overflow-y-auto">
            <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Crear Grupo de Etiquetas</h2>
            <form @submit.prevent="submitForm">
                <!-- Campo Nombre -->
                <div class="mb-6">
                    <label class="block text-md font-semibold text-gray-700 mb-2">Nombre</label>
                ...
                <div class="flex gap-8">
                    ...
                    <div class="w-1/2 bg-gray-50 border border-gray-300 rounded-lg shadow p-4">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" @click="selectAll('available')"
                                class="text-#497fae-600 hover:text-#497fae-800 font-medium">
                                <i class="mdi mdi-checkbox-multiple-marked-outline text-xl"></i>
                            </button>

                        </div>
                        <div class="border border-gray-200 rounded-lg p-4 min-h-[300px] bg-white overflow-y-auto"
                            @dragover.prevent @drop="dropItem('available')">
                            <div v-for="tag in availableTags" :key="tag._id"
                                class="bg-#497fae-50 border border-#497fae-200 rounded-lg p-2 mb-2 shadow-md cursor-pointer hover:bg-#497fae-100 transition"
                                draggable="true" @dragstart="dragStart(tag)" @click="toggleSelection(tag, 'available')">
                                {{ tag.name }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-1/2 bg-gray-50 border border-gray-300 rounded-lg shadow p-4">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" @click="selectAll('selected')"
                                class="text-orange-600 hover:underline font-medium">
                                Quitar Todos
                            </button>
                        </div>
                        <div class="border border-gray-200 rounded-lg p-4 min-h-[300px] bg-white overflow-y-auto"
                            @dragover.prevent @drop="dropItem('selected')">
                            <div v-for="tag in selectedTags" :key="tag._id"
                                class="bg-[#6999c2]-50 border border-orange-200 rounded-lg p-2 mb-2 shadow-md cursor-pointer hover:bg-[#6999c2]-100 transition"
                                draggable="true" @dragstart="dragStart(tag)" @click="toggleSelection(tag, 'selected')">
                                {{ tag.name }}
                            </div>
                        </div>
                    </div>
